...
export default {
  ...
  computed: {
    ...mapGetters(['getSelectedSemester', 'getExamDates', 'getRealNote', 'getLocalisation', 'getCourseByNote', 'isDarkMode', 'getCalendarEvents']),

    examDates: {
      get() {
        let semester = this.getSelectedSemester
        let exams = this.getExamDates(semester)
        let result = []
